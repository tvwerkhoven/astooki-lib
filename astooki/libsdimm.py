...
def mergeMap(covmaps, slists, alists):
	"""
	...
	for al in alists:
		alist.extend(al)
	N.lib.arraysetops.unique1d(alist)
	N.lib.arraysetops.unique1d(slist)
	...
	for n in range(len(covmaps)):
		...
		for _s in range(cmap.shape[1]):
			...
			for _a in range(cmap.shape[2]):
				...
				covmap += cmap[:-1,_s,_a] * cmap[-1,_s,_a] 
				covmap += cmap[-1,_s,_a] 
	
	# Normalize the map
	covmap[:-1] /= covmap[-1].reshape(1,covmap.shape[1],covmap.shape[2])
			...
			for colsa1 in salist:
				...
				for colsa2 in othersa:
					...
					log.prNot(log.NOTICE, "COL: sa %d @ (%g,%g) <-> sa %d @ (%g,%g)."% \
						(colsa1, ) + tuple(colsa1) + (colsa2, ) + tuple(colsa2))
